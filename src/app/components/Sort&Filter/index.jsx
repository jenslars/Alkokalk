...
const SortFilter = ({ onSort, onFilter, onSearch, onReset }) => {
  ...
  return (
    <StyledSortFilterContainer>
      ...
      <StyledText>
        Systembolagets drycker sorterade efter APK (alkohol per krona)
      </StyledText>
      <ButtonContainer>
        <StyledButton onClick={handleSortClick}>
          Sortera enligt {isDescending ? "stigande" : "fallande"} APK
        </StyledButton>
        <Select
      ...
      <StyledText>
        Sorteringstyp:{" "}
        {isDescending
          ? "Från högst APK till lägst"
          : "Från lägst APK till högst"}
      </StyledText>
    </StyledSortFilterContainer>
  );
};
